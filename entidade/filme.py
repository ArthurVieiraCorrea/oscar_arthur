...
class Filme:
    def __init__(self, nome: str, ano: int, nacionalidade: str, diretor: str): 
        self.__nome = nome
        self.__ano = ano
    ...
    @ano.setter
    def novo_ano (self,novo_ano: int):
    ...
    def novo_diretor (self, novo_diretor: str):
        if isinstance (novo_diretor, str):
            ...
            raise TypeError("Novo diretor deve ser uma string.")
    ...
    def mostrar_dados (self):
        return f"Filme: {self.nome} - Ano: {self.ano} - Nacionalidade: {self.nacionalidade} - Diretor: {self.diretor} - indicações: {len(self.indicacoes)} "